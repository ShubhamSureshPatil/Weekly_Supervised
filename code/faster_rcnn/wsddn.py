...
class WSDDN(nn.Module):
    ...
    def __init__(self, classes=None, debug=False, training=True):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(inplace=True))
        self.score_1_cls = nn.Linear(in_features=4096, out_features=20)
    ...
    def forward(self,
        ...
        x = self.classifier(x)
        score_1_cls = self.score_1_cls(x)
        score_det = self.score_det(x)

        score_1_cls = F.softmax(score_1_cls, dim=1)
        score_det = F.softmax(score_det, dim=0)
        prob_class = score_1_cls * score_det  
    ...
    def build_loss(self, cls_prob, label_vec):
        ...
        pre_label = torch.sum(prob_class, dim=0, keepdim=True)
