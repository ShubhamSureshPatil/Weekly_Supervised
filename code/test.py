...
def test_net(name,
    ...
    def metric(output, target):
        bs, score_cls = target.shape
        map_all_inclusive = []

        for i in range(score_cls):
        
            tar_cls = target[:, i]
            out_cls = output[:, i]
        
            out_cls -= 1e-5 * tar_cls
            map = sklearn.metrics.average_precision_score(tar_cls, out_cls, average=None)
            map_all_inclusive.append(map)
        
        ap_mean = np.sum(map_all_inclusive) / float(score_cls)
        
        return ap_mean, map_all_inclusive

    ap_mean, map_all_inclusive = metric(scores_all, gt_all)
